...
## Global options:
knitr::opts_chunk$set(cache = FALSE, # Set this to TRUE if chunks are performing time taking calculations and you want to use those calculations at a later stage. The first time chunks run, R will store the results of chunk in your disk and use this copy for all subsequent runs
...
```{r mps-ttest}
t_test <- t.test(mps$lnrealgross[mps$treated ==  1],
...
CreateTableOne(vars = c('labour', 'tory', 'yob','yod',
...
out <- data.frame(
  ...
  "raw" = c(paste0(tory$p.label," (",round(tory$std.error,2),")"),
            paste0(aristo$p.label," (",round(aristo$std.error,2),")"),
            paste0(scat_eto$p.label," (",round(scat_eto$std.error,2),")")),
  "match" = c(paste0(tory.match$p.label," (",round(tory.match$std.error,2),")"),
...
dta_plot = rbind(
  data.frame(var = rownames(summary(match.out)$sum.all),
             type = 'raw',
           std = summary(match.out)$sum.all[, 'Std. Mean Diff.']),
  data.frame(var = rownames(summary(match.out)$sum.matched),
             type = 'matched',
...
ggplot(dta_plot,
  ...
  labs(title = 'Standadrized Mean Differences',
...
```{r extend}
## matching
match.out2 <- matchit(treated ~ tory + aristo + scat_pub + scat_eto +
...
## matching labour
match.lab <- matchit(treated ~ tory + aristo + scat_pub + scat_eto +
                       scat_nm + scat_sec + oc_teacherall + oc_barrister +
                       oc_solicitor + oc_dr + oc_civil_serv +
                       oc_local_politics + oc_business + oc_white_collar +
                       oc_union_org + oc_journalist + oc_miner + ucat_ox +
...
## matching tories
match.tory <- matchit(treated ~ tory + aristo + scat_pub + scat_eto +
                        scat_nm + scat_sec + oc_teacherall + oc_barrister +
                        oc_solicitor + oc_dr + oc_civil_serv +
                        oc_local_politics + oc_business + oc_white_collar +
                        oc_union_org + oc_journalist + oc_miner + ucat_ox +
...
## labour
match.gen.lab <- matchit(treated ~ tory + aristo + scat_pub + scat_eto +
                           scat_nm + scat_sec + oc_teacherall + oc_barrister +
                           oc_solicitor + oc_dr + oc_civil_serv +
                           oc_local_politics + oc_business + oc_white_collar +
                           oc_union_org + oc_journalist + oc_miner + ucat_ox +
...
## tories
match.gen.tory <- matchit(treated ~ tory + aristo + scat_pub + scat_eto +
                            scat_nm + scat_sec + oc_teacherall + oc_barrister +
                            oc_solicitor + oc_dr + oc_civil_serv +
                            oc_local_politics + oc_business + oc_white_collar +
                            oc_union_org + oc_journalist + oc_miner + ucat_ox +
