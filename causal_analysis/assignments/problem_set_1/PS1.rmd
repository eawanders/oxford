...
file.exists("Vietnam_matching-1.dta")
...
covariates_table <- data.frame(
  ...
  Explanation = c(
    ...
    "Hamlets closer to international borders may have strategic importance affecting
    both bombing decisions and control.", "More developed hamlets may have been targeted
    differently and had different control conditions.", "Larger hamlets may be more
    strategically important, affecting both bombing decisions and control dynamics.",
    "If a hamlet was already contested or insurgent-controlled before bombing,
    this could affect both the likelihood of being bombed and later control."
  )
)
...
nearest_neighbour_matching <- matchit(
  bombed_969_bin ~ std + ln_dist + score + lnhpop + mod2a_1admn,
  data = vietnam_data,
  method = "nearest",
  estimand = "ATT",
  replace = FALSE
...
att_model <- lm(
  mod2a_1adec ~ bombed_969 + std + ln_dist + score + lnhpop + mod2a_1admn,
  data = matched_data,
  weights = weights
)
...
logistic_att_model <- polr(
  factor(mod2a_1adec) ~
    bombed_969 + std + ln_dist + score + lnhpop + mod2a_1admn,
  data = matched_data,
  weights = weights,
  method = "logistic"
)
