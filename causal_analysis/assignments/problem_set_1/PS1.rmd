...
modelsummary(models,
  ...
  Both ATT models use matched data to estimate the causal effect of ``bombed_969`` on ``mod2a_1adec``.",
)

# Extract coefficients for bombed_969
ols_coef <- coef(ols_model)["bombed_969"] # OLS model coefficient
att_coef <- coef(att_model)["bombed_969"] # ATT model coefficient
logit_or <- exp(coef(logistic_att_model)["bombed_969"]) # Convert log-odds to odds ratio
```
