...
covariates_table <- data.frame(
  ...
  Explanation = c(
    ...
    "Larger hamlets may be more strategically important, affecting both bombing decisions and control dynamics.",
...
This model's balance is then asssessed across covariates using the Mean Standardised Difference before and after matching, shown in Figure 1. We see from this figure that all post-matching covariates fall within the accepted range of balance of ``<0.1`` making this a suitable matching estimator for the dataset.
...
results_table %>%
  ...
  footnote(
    general = "MASD = Mean Absolute Standardized Difference, MSMD = Median Standardized Mean Difference, MaxSMD = Maximum Standardized Mean Difference. Lower values indicate better balance.",
    threeparttable = TRUE
  )
```

```{r Matching Plot, echo=FALSE}
# Extract balance statistics for Mahalanobis Matching (With Replacement, Ratio = 2)
match.out <- mahalanobis_matching_ratio2 # Assign model to variable

# Combine relevant statistics
dta_plot <- rbind(
  data.frame(
    var = rownames(summary(match.out, standardize = TRUE)$sum.all),
    type = "Raw",
    std = summary(match.out, standardize = TRUE)$sum.all[, "Std. Mean Diff."]
  ),
  data.frame(
    var = rownames(summary(match.out, standardize = TRUE)$sum.matched),
    type = "Matched",
    std = summary(match.out, standardize = TRUE)$sum.matched[, "Std. Mean Diff."]
  )
)

# Define cutoff values (typically 0.1 or 0.25 for SMD thresholds)
cutoff1 <- 0.05
cutoff2 <- 0.1

# Generate SMD Plot with Requested Modifications
ggplot(dta_plot, aes(y = var, x = std, shape = type, color = type)) +
  geom_vline(aes(xintercept = 0), color = "black", linewidth = 0.25) + # Vertical line at 0
  geom_vline(aes(xintercept = cutoff1), linetype = "dashed", color = "black", linewidth = 0.25) + # Cutoff at 0.05
  geom_vline(aes(xintercept = -cutoff1), linetype = "dashed", color = "black", linewidth = 0.25) +
  geom_vline(aes(xintercept = cutoff2), linetype = "solid", color = "black", linewidth = 0.25) + # Cutoff at 0.1
  geom_vline(aes(xintercept = -cutoff2), linetype = "solid", color = "black", linewidth = 0.25) +
  geom_vline(aes(xintercept = -1), linetype = "solid", color = "black", linewidth = 0.25) + # X-axis boundary
  geom_hline(aes(yintercept = 0), linetype = "solid", color = "black", linewidth = 0.25) +
  geom_point(size = 2, stroke = 0.5, shape = 1, color = "black") + # Unmatched (outline only)
  geom_point(data = dta_plot[dta_plot$type == "Matched", ], aes(x = std, y = var), size = 1.75, fill = "black") + # Matched (solid black)
  scale_shape_manual(values = c("Unmatched" = 1, "Matched" = 16)) + # Open circles (Unmatched) and filled circles (Matched)
  scale_color_manual(values = c("Unmatched" = "black", "Matched" = "black")) + # Ensure both appear in legend
  scale_x_continuous(limits = c(-1, 1), breaks = seq(-1, 1, by = 0.25)) + # X-axis scale from -1 to +1
  scale_y_discrete(expand = c(0.2, 0.2)) + # Remove extra space on y-axis
  scale_color_manual(values = c("black", "black")) + # Ensure both have black color
  theme_minimal(base_family = "Times") +
  labs(
    x = "Standardized Mean Differences",
    y = "",
    caption = "Note: Figure represents the Mahalanobis Matching (With Replacement, Ratio = 2) model. Lower values indicate better balance. \n Black dots represents post-matching standardized mean differences and open circles represent pre-matching standardized mean differences."
  ) +
  theme(
    legend.title = element_blank(), # Remove legend title
    legend.position = "none", # Keep legend on the right
    plot.caption = element_text(hjust = 0), # Left-align caption
    text = element_text(family = "Times", size = 10), # Set consistent font
    axis.text = element_text(family = "Times", size = 10), # Reduce x-axis text size
    axis.title.x = element_text(size = 10, margin = margin(t = 5)), # Reduce x-axis title size and increase spacing
    axis.ticks = element_line(color = "black", linewidth = 0.25), # Match table lines
    panel.grid.major.x = element_blank(), # Remove vertical grid lines
    panel.grid.major.y = element_line(linetype = "dashed", color = "grey70", linewidth = 0.25), # Soft dashed horizontal lines
    panel.grid.minor = element_blank(), # Remove minor grid lines
    plot.margin = margin(10, 10, 30, 10) # Increase bottom margin for x-axis spacing
  )
```
