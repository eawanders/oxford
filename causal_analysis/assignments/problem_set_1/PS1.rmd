...
When using matching methods to estimate causal effects, we want to compare a treatment group to a control group that is as similar as possible to the treatment group. This is done by matching units on covariates that are likely to be associated with both the treatment and the outcome.

In this case, we should use covariates that are likely to be associated with both the bombing in September 1969 and the control of hamlets by the Viet Cong in December 1969. These covariates should be measured before the treatment is applied to avoid bias in the estimation of the treatment effect.