...
source("../writing/build_outputs.R")
```
...
To test what drives this possible affective polarisation, the models for the outcome variables of `MLthermoMean` and `LLthermoMean` are estimated. These models are presented in the Appendix in \hyperref[tab:thermo-ml-results]{Table \ref*{tab:thermo-ml-results}} and \hyperref[tab:thermo-ll-results]{Table \ref*{tab:thermo-ll-results}}. Thermometer scores for both in- and out-party leaders show positive increases of **``r paste0(round(coef(summary(full_thermo_ml_model))["ai_treatment", "Estimate"], 3), " points")``** and **``r paste0(round(coef(summary(full_thermo_ll_model))["ai_treatment", "Estimate"], 3), " points")``** respectively to the treatment but the scores are not statistically significant. Yet, when looking at significant subgroup interaction terms, the increased rating gap between the in- and out-party leaders appears driven by an increased warmth towards the in-party leader, rather than a decrease in warmth towards the out-party leader, shown in \hyperref[fig:thermo-patchwork-ai]{Figure \ref*{fig:thermo-patchwork-ai}}.[^low-education-significance] While the interaction effects for the `MLthermoMean` and `LLthermoMean` party leaders are not statistically significant, the results suggest the direction of possible of effects remains interesting. The divide between in- and out-group ratings appears driven by an increased warmth towards the in-party, rather than the decreased rating of the opposition which was expected and reported by @iyengar2012 in the US.
...
<!--
These results are visualised in \hyperref[fig:ordinal-patchwork-ai]{Figure \ref*{fig:ordinal-patchwork-ai}} which shows the predicted probabilities of the ordinal measures of affective polarisation by sub-group.[^preds-model-measurement] The predicted probabilities are calculated as the sum of the probability of a respondent being at the polarised end of the ordinal outcome levels, meaning a higher probability indicates greater affective polarisation.[^summed-preds] The included levels show the statistically significant sub-groups for *respect* and *discomfort* measures, indicating those who are most susceptible to being polarised by the treatment.[^ai-subgroups] In particular, low-educated, part-time-working respondents are significantly more likely to express more polarised attitudes towards their out-party. The greatest effect size is seen when respondents consider their discomfort with having a child marry an out-party voter, with the predicted probability of discomfort for those low-educated in the treatment group being **`r paste0(round(low_edu_treatment, 3) * 100, " %")`** compared to **`r paste0(round(low_edu_control, 3) * 100, " %")`** for the control group. This is/is not to be expected due to...
-->
