...
```{r thermo-gap-results-table, echo=FALSE, message=FALSE, warning=FALSE, results='hid'}
...
```{r thermo-plot-overall, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
## Helper function to extract control mean and treatment effect
get_means <- function(model, treatment_var = "ai_treatment") {
    coefs <- coef(model)
...
ml_df_overall <- get_means(full_thermo_ml_model) %>% mutate(Outcome = "MLthermoMean")
ll_df_overall <- get_means(full_thermo_ll_model) %>% mutate(Outcome = "LLthermoMean")
gap_df_overall <- get_means(full_thermo_gap_model) %>% mutate(Outcome = "thermo_gap")

thermo_plot_data_overall <- bind_rows(ml_df_overall, ll_df_overall, gap_df_overall) %>%
    mutate(Outcome = factor(Outcome, levels = c("MLthermoMean", "LLthermoMean", "thermo_gap")))

plot_overall <- ggplot(thermo_plot_data_overall, aes(x = Group, y = Mean, group = Outcome, colour = Outcome)) +
    geom_line(size = 0.75) +
    geom_point(size = 2) +
    scale_colour_manual(
        values = c("MLthermoMean" = "#4b4b4b", "LLthermoMean" = "#bdbdbd", "thermo_gap" = "black"),
        labels = c("Most Likely", "Least Likely", "Thermometer Gap")
    ) +
    labs(
        x = NULL,
...
```{r thermo-plot-ld, echo=FALSE, message=FALSE, results='hide'}
# === Thermometer Models Results Graphs for Liberal Democrat subgroup ===

# Updated function to extract means for the Lib Dem subgroup
get_libdem_means <- function(model, treatment_var = "ai_treatment", interaction_term = "ai_treatment:mostlikelyLiberal Democrats") {
    coefs <- coef(model)
    intercept <- coefs["(Intercept)"]
    treatment_effect <- coefs[treatment_var]
    interaction_effect <- ifelse(interaction_term %in% names(coefs), coefs[interaction_term], 0)
    tibble(
        Group = c("Control", "Treatment"),
...
ml_df_libdem <- get_libdem_means(full_thermo_ml_model) %>% mutate(Outcome = "MLthermoMean")
ll_df_libdem <- get_libdem_means(full_thermo_ll_model) %>% mutate(Outcome = "LLthermoMean")
gap_df_libdem <- get_libdem_means(full_thermo_gap_model) %>% mutate(Outcome = "thermo_gap")

thermo_plot_data_libdem <- bind_rows(ml_df_libdem, ll_df_libdem, gap_df_libdem) %>%
    mutate(Outcome = factor(Outcome, levels = c("MLthermoMean", "LLthermoMean", "thermo_gap")))

plot_libdem <- ggplot(thermo_plot_data_libdem, aes(x = Group, y = Mean, group = Outcome, colour = Outcome)) +
    geom_line(size = 0.75) +
    geom_point(size = 2) +
    scale_colour_manual(
        values = c("MLthermoMean" = "#4b4b4b", "LLthermoMean" = "#bdbdbd", "thermo_gap" = "black"),
    ...
    labs(
        x = NULL,
        y = "Feeling Thermometer (Liberal Democrat Subgroup)",
        colour = NULL
    ) +
    theme_classic(base_family = "serif") +
    ylim(0, 100) +
...
```{r thermo-plots-green, echo=FALSE, message=FALSE, results='hide'}
# === Thermometer Models Results Graphs for Green Party subgroup ===

# Updated function to extract means for the Green Party subgroup
get_green_means <- function(model, treatment_var = "ai_treatment", interaction_term = "ai_treatment:mostlikelyGreen Party") {
    coefs <- coef(model)
    intercept <- coefs["(Intercept)"]
    treatment_effect <- coefs[treatment_var]
    interaction_effect <- ifelse(interaction_term %in% names(coefs), coefs[interaction_term], 0)
...
ml_df_green <- get_green_means(full_thermo_ml_model) %>% mutate(Outcome = "MLthermoMean")
ll_df_green <- get_green_means(full_thermo_ll_model) %>% mutate(Outcome = "LLthermoMean")
gap_df_green <- get_green_means(full_thermo_gap_model) %>% mutate(Outcome = "thermo_gap")

thermo_plot_data_green <- bind_rows(ml_df_green, ll_df_green, gap_df_green) %>%
    mutate(Outcome = factor(Outcome, levels = c("MLthermoMean", "LLthermoMean", "thermo_gap")))

plot_green <- ggplot(thermo_plot_data_green, aes(x = Group, y = Mean, group = Outcome, colour = Outcome)) +
    geom_line(size = 0.75) +
    geom_point(size = 2) +
    scale_colour_manual(
        values = c("MLthermoMean" = "#4b4b4b", "LLthermoMean" = "#bdbdbd", "thermo_gap" = "black"),
    ...
    labs(
        x = NULL,
        y = "Feeling Thermometer (Green Party Subgroup)",
        colour = NULL
    ) +
    theme_classic(base_family = "serif") +
    ylim(0, 100) +
...
```{r thermo-plot-educ, echo=FALSE, message=FALSE, results='hide'}
# === Thermometer Models Results Graphs for Low Education subgroup ===

# Function to extract means for Low Education subgroup
get_lowedu_means <- function(model, treatment_var = "ai_treatment", interaction_term = "ai_treatment:education_recodeLow") {
    coefs <- coef(model)
    intercept <- coefs["(Intercept)"]
    treatment_effect <- coefs[treatment_var]
    interaction_effect <- ifelse(interaction_term %in% names(coefs), coefs[interaction_term], 0)
...
ml_df_lowedu <- get_lowedu_means(full_thermo_ml_model) %>% mutate(Outcome = "MLthermoMean")
ll_df_lowedu <- get_lowedu_means(full_thermo_ll_model) %>% mutate(Outcome = "LLthermoMean")
gap_df_lowedu <- get_lowedu_means(full_thermo_gap_model) %>% mutate(Outcome = "thermo_gap")

thermo_plot_data_lowedu <- bind_rows(ml_df_lowedu, ll_df_lowedu, gap_df_lowedu) %>%
    mutate(Outcome = factor(Outcome, levels = c("MLthermoMean", "LLthermoMean", "thermo_gap")))

plot_lowedu <- ggplot(thermo_plot_data_lowedu, aes(x = Group, y = Mean, group = Outcome, colour = Outcome)) +
    geom_line(size = 0.75) +
    geom_point(size = 2) +
    scale_colour_manual(
        values = c("MLthermoMean" = "#4b4b4b", "LLthermoMean" = "#bdbdbd", "thermo_gap" = "black"),
    ...
    labs(
        x = NULL,
        y = "Feeling Thermometer (Low Education Subgroup)",
        colour = NULL
    ) +
    theme_classic(base_family = "serif") +
    ylim(0, 100) +
...
```{r themo-plots-combined, echo=FALSE, message=FALSE, results='hide', fig.cap="\\label{fig:thermo-patchwork} Thermometer Scores by Subgroup"}
library(patchwork)

# 1. Standardise each plot
standardise_plot <- function(p, ylab = "Thermometer Score") {
    p +
...
plot_overall <- standardise_plot(plot_overall) + ggtitle("Average Treatment Effect")
plot_libdem <- standardise_plot(plot_libdem) + ggtitle("Liberal Democrat Subgroup")
plot_green <- standardise_plot(plot_green) + ggtitle("Green Party Subgroup")
plot_lowedu <- standardise_plot(plot_lowedu) + ggtitle("Low Education Subgroup")

# 2. Combine with patchwork, add label
combined_thermo_plot <- (
    (plot_overall + plot_libdem) /
        (plot_green + plot_lowedu)
) +
    plot_layout(guides = "collect") &
    theme(legend.position = "bottom")

# 3. Print the final patchwork figure
print(combined_thermo_plot)
```
...
xtrust_full <- ordinal_models(
    ...
    moderators = c("mostlikely", "education_recode")
)[[1]]
...
```{r agreedisagree-probability-plot, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.cap="Predicted Probability of Disagreement that Out-Parties respect In-Party Beliefs by Moderator"}
# 1. Estimate polr model with interactions
polr_model <- polr(
    agreedisagree ~ ai_treatment * education_recode + ai_treatment * profile_work_stat,
    data = yougov_data,
    method = "logistic",
    Hess = TRUE
)

# === Individual collapsed predictions ===

# Helper to extract and collapse predictions for a single subgroup
collapse_preds <- function(term_list, subgroup_label) {
    preds_filtered <- ggpredict(polr_model, terms = term_list) %>%
        filter(response.level %in% c("Strongly disagree", "Tend to disagree"))

    preds_collapsed <- preds_filtered %>%
        group_by(x) %>%
        summarise(
            predicted = sum(predicted),
            conf.low = sum(predicted) - sqrt(sum((predicted - conf.low)^2)),
            conf.high = sum(predicted) + sqrt(sum((conf.high - predicted)^2)),
            .groups = "drop"
        )

    preds_collapsed %>%
        mutate(
            treatment = ifelse(x == 0, "Control", "Treatment"),
            mod_group = subgroup_label
        )
}

# Low Education
preds_edu <- collapse_preds(
    term_list = c("ai_treatment", "education_recode[Low]"),
    subgroup_label = "Low Education"
)

# Working PT (8–29h)
preds_work1 <- collapse_preds(
    term_list = c("ai_treatment", "profile_work_stat[Working part time (8-29 hours a week)]"),
    subgroup_label = "Working PT (8–29h)"
)

# Working PT (<8h)
preds_work2 <- collapse_preds(
    term_list = c("ai_treatment", "profile_work_stat[Working part time (Less than 8 hours a week)]"),
    subgroup_label = "Working PT (<8h)"
)

# Overall
preds_overall <- ggpredict(polr_model, terms = "ai_treatment") %>%
    filter(response.level %in% c("Strongly disagree", "Tend to disagree")) %>%
    group_by(x) %>%
    summarise(
        predicted = sum(predicted),
...
# === Combine all results ===
final_preds <- bind_rows(preds_edu, preds_work1, preds_work2, preds_overall)

final_preds <- final_preds %>%
    mutate(mod_group = factor(mod_group, levels = c(
        "Overall",
        "Low Education",
        "Working PT (<8h)",
        "Working PT (8–29h)"
...
overall_treatment_value <- final_preds %>%
    filter(mod_group == "Overall", treatment == "Treatment") %>%
    pull(predicted)

# === Plot ===
position <- position_dodge(width = 0.5)

plot_disagreement <- ggplot(final_preds, aes(x = mod_group, y = predicted, group = treatment, shape = treatment)) +
    geom_point(size = 3, position = position) +
    geom_hline(yintercept = overall_treatment_value, linetype = "dashed", colour = "black") +
    labs(
        x = "Moderator Subgroup",
...
```{r xtrust-probability-plot, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.cap="Predicted Probability of Low Trust in Out-Parties by Moderator"}
# Estimate polr model with interactions
polr_model <- polr(
    xtrust ~ ai_treatment * mostlikely + ai_treatment * education_recode,
    data = yougov_data,
    method = "logistic",
    Hess = TRUE
)

# Helper to extract and collapse predictions
collapse_preds <- function(term_list, subgroup_label) {
    preds_filtered <- ggpredict(polr_model, terms = term_list) %>%
        filter(response.level %in% c("Almost never", "Once in a while"))
...
# Labour Voters
preds_lab <- collapse_preds(
    term_list = c("ai_treatment", "mostlikely[Labour Party]"),
    subgroup_label = "Labour"
)

# Lib Dem Voters
preds_ld <- collapse_preds(
    term_list = c("ai_treatment", "mostlikely[Liberal Democrats]"),
    subgroup_label = "Liberal Democrats"
)
...
# Combine
final_preds <- bind_rows(preds_lab, preds_ld, preds_edu, preds_overall) %>%
    mutate(
        mod_group = factor(mod_group, levels = c("Overall", "Labour", "Liberal Democrats", "Low Education"))
    )
...
plot_trust <- ggplot(final_preds, aes(x = mod_group, y = predicted, group = treatment, shape = treatment)) +
    geom_point(size = 3, position = position) +
    geom_hline(yintercept = overall_treatment_value, linetype = "dashed", colour = "black") +
    labs(
        x = "Moderator Subgroup",
        y = "Predicted Probability of Low Trust",
        shape = "Treatment"
    ) +
    scale_shape_manual(values = c(1, 19)) +
    ylim(0, 1) +
...
```{r child-probability-plot, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.cap="Predicted Probability of Discomfort with Child Marrying Opposing Partisan by Moderator"}
# 1. Estimate polr model with interactions
polr_model <- polr(
    child ~ ai_treatment * education_recode + ai_treatment * profile_GOR,
    data = yougov_data,
    method = "logistic",
    Hess = TRUE
)

# === Helper to extract and collapse predicted disagreement ===
collapse_preds <- function(term_list, subgroup_label) {
    preds_filtered <- ggpredict(polr_model, terms = term_list) %>%
        filter(response.level %in% c("Extremely upset", "Somewhat upset"))
...
# North East
preds_ne <- collapse_preds(
    term_list = c("ai_treatment", "profile_GOR[North East]"),
    subgroup_label = "North East"
)

# South East
preds_se <- collapse_preds(
    term_list = c("ai_treatment", "profile_GOR[South East]"),
    subgroup_label = "South East"
)

# Overall (no moderator)
preds_overall <- ggpredict(polr_model, terms = "ai_treatment") %>%
    filter(response.level %in% c("Extremely upset", "Somewhat upset")) %>%
    group_by(x) %>%
    summarise(
...
# Combine
final_preds <- bind_rows(preds_edu, preds_ne, preds_se, preds_overall) %>%
    mutate(
        mod_group = factor(mod_group, levels = c("Overall", "Low Education", "North East", "South East"))
    )
...
plot_discomfort <- ggplot(final_preds, aes(x = mod_group, y = predicted, group = treatment, shape = treatment)) +
    geom_point(size = 3, position = position) +
    geom_hline(yintercept = overall_treatment_value, linetype = "dashed", colour = "black") +
    labs(
        x = "Moderator Subgroup",
        y = "Predicted Probability of Discomfort",
        shape = "Treatment"
    ) +
    scale_shape_manual(values = c(1, 19)) +
    ylim(0, 1) +
...
```{r combined-ordinal-plots, echo=FALSE, message=FALSE, results='asis', fig.cap="Predicted Probabilities of Affective Polarisation Measures of Respect, Trust, and Discomfort"}
# Apply individual titles and adjustments to each plot
plot_disagreement <- plot_disagreement +
    ggtitle("Respect Beliefs") +
    theme(
...
plot_discomfort <- plot_discomfort +
    ggtitle("Discomfort with Marriage") +
    theme(
        plot.title = element_text(
            size = 9,
            family = "serif",
...
# Combine them horizontally with shared legend at bottom
...
```{r codebook-table, echo=FALSE, message=FALSE, warning=FALSE}
# === Codebook ===
